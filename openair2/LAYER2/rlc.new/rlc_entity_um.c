    ...
    rlc_rx_pdu_segment_t *pdu)
{
  ...
  if (sn != (r->sn + 1) % entity->sn_modulus ||
      !(fi & 0x02)) {
    if (r->sdu_pos)
      printf("%s:%d:%s: warning: discard partially reassembled SDU\n",
...
#define R(d) do { if (rlc_pdu_decoder_in_error(&d)) goto err; } while (0)
  ...
  if ((sn_compare_rx(entity, entity->vr_ur, sn) < 0 &&
    ...
    printf("%s:%d:%s: warning: discard PDU (sn %d vr(ur) %d vr(uh) %d\n",
           __FILE__, __LINE__, __FUNCTION__,
...
printf("got SDU size %d entity->tx_size %d entity->tx_maxsize %d\n",
       size, entity->tx_size, entity->tx_maxsize);
